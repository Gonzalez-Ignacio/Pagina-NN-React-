...
export const AuthProvider = ({ children }) => {       {/* AuthProvider (COMPONENTE) es el que le da contexto a toda la App. Éste es el que usamos para renderizar en main.jsx. Se le da el children para que todos los hijos tengan acceso al contexto de autentificación. */}
  ...
  //LocalStorage para recordar al usuario que inicio sesión.
  useEffect(() => {
    const loguearUsuarioJSON = window.localStorage.getItem("loguearUsuario");       
    if(loguearUsuarioJSON) {                                                       
        const usuario = JSON.parse(loguearUsuarioJSON);                             
        if(usuario.email && usuario.role) {setUserRole(usuario.role)};              
    }                                                                               
}, []);
